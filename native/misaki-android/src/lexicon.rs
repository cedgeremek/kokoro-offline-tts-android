...
const GB_GOLD_JSON: &str = include_str!("../data/gb_gold.json");
const GB_SILVER_JSON: &str = include_str!("../data/gb_silver.json");
...
impl Lexicon {
    /// Create the default US Misaki lexicon.
    pub fn new() -> Self {
        Self::new_for_accent(false)
    }

    /// Create the matching US or GB Misaki lexicon.
    pub fn new_for_accent(british: bool) -> Self {
        let (gold_json, silver_json) = if british {
            (GB_GOLD_JSON, GB_SILVER_JSON)
        } else {
            (US_GOLD_JSON, US_SILVER_JSON)
        };
        let golds_raw: HashMap<String, LexEntry> =
            serde_json::from_str(gold_json).expect("failed to parse gold lexicon");
        let silvers_raw: HashMap<String, LexEntry> =
            serde_json::from_str(silver_json).expect("failed to parse silver lexicon");
    ...
    fn suffix_s(&self, stem: &str) -> Option<String> {
        ...
        if "ptkfθ".contains(last) {
            ...
            Some(format!("{}{}z", stem, if self.british { 'ɪ' } else { 'ᵻ' }))
        } else {
            Some(format!("{}z", stem))
        }
    }
    ...
    fn suffix_ed(&self, stem: &str) -> Option<String> {
        ...
        if "pkfθʃsʧ".contains(last) {
            ...
            Some(format!("{}{}d", stem, if self.british { 'ɪ' } else { 'ᵻ' }))
        } else if last != 't' {
            Some(format!("{}d", stem))
        } else {
            let chars: Vec<char> = stem.chars().collect();
            if self.british || chars.len() < 2 {
                Some(format!("{}ɪd", stem))
            } else {
    ...
    fn suffix_ing(&self, stem: &str) -> Option<String> {
        ...
        let chars: Vec<char> = stem.chars().collect();
        if self.british && matches!(chars.last(), Some('ə' | 'ː')) {
            return None;
        }
        if chars.len() > 1
            && !self.british
