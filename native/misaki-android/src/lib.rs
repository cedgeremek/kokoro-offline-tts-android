...
fn global_g2p(british: bool) -> &'static G2P {
    static US_INSTANCE: OnceLock<G2P> = OnceLock::new();
    static GB_INSTANCE: OnceLock<G2P> = OnceLock::new();
    if british {
        GB_INSTANCE.get_or_init(|| G2P::new_for_accent(true))
    } else {
        US_INSTANCE.get_or_init(G2P::new)
    }
}
...
impl G2P {
    ...
    pub fn new_for_accent(british: bool) -> Self {
        Self::with_config(G2PConfig {
            british,
            ..G2PConfig::default()
        })
    }
    ...
    pub fn with_config(config: G2PConfig) -> Self {
        Self {
            lexicon: Lexicon::new_for_accent(config.british),
            fallback: EspeakFallback::for_accent(config.british),
            unk: String::new(),
            overrides: HashMap::new(),
        }
    }

    // Historical voice-g2p product overrides are retained as source provenance
    // only. They are deliberately unbuildable so current Misaki data/eSpeak is
    // the sole default behavior.
    #[cfg(any())]
    ...
    fn resolve_group(&self, group: &mut [MToken], ctx: &TokenContext) {
        ...
        while left < right {
            ...
            if let Some(ps) = ps {
                let merged = merge_tokens(&group[left..right], None);
                group_ctx = Self::token_context(&group_ctx, Some(&ps), &merged);
        ...
        if should_fallback {
            // Official Misaki sends the complete merged subtoken group through
            // its fallback. Resolving pieces independently changes acronyms,
            // hyphenated words, identifiers, and unknown compounds.
            let merged = merge_tokens(group, None);
            if let Some((phonemes, rating)) = self.fallback.convert_word(&merged.text) {
                group[0].phonemes = Some(phonemes);
                group[0].underscore.rating = Some(rating);
                for tk in group.iter_mut().skip(1) {
                    tk.phonemes = Some(String::new());
                    tk.underscore.rating = Some(rating);
                }
            }
        } else {
...
pub fn english_to_phonemes(text: &str) -> Result<String, G2pError> {
    global_g2p(false).convert(text)
}

pub fn english_to_phonemes_for_accent(text: &str, british: bool) -> Result<String, G2pError> {
    global_g2p(british).convert(text)
}

#[no_mangle]
pub extern "system" fn Java_com_local_kokorotts_NativeMisaki_initialize(
    mut env: JNIEnv,
    _class: JClass,
    ...
    let result = (|| -> Result<(), String> {
        let library_path: String = env
            .get_string(&library_path)
            .map_err(|error| error.to_string())?
            .into();
        let data_path: String = env
            .get_string(&data_path)
            .map_err(|error| error.to_string())?
            .into();
        espeak::initialize(&library_path, &data_path)
    })();
    match result {
        Ok(()) => std::ptr::null_mut(),
        Err(error) => env
...
#[no_mangle]
pub extern "system" fn Java_com_local_kokorotts_NativeMisaki_phonemize(
    mut env: JNIEnv,
    _class: JClass,
    text: JString,
    british: jboolean,
    ...
    let result = (|| -> Result<String, String> {
        let text: String = env
            ...
            .map_err(|error| error.to_string())?
            .into();
        english_to_phonemes_for_accent(&text, british != 0).map_err(|error| error.to_string())
    })();
    match result {
        Ok(phonemes) => env
            .new_string(phonemes)
...
mod tests {
    ...
    #[test]
    fn test_g2p_hyphen_compound_matches_current_misaki_joining() {
        let g2p = G2P::new();
        let result = g2p.convert("cat-dog").unwrap();
        assert!(!result.contains(' '), "Unexpected compound space: {result}");
    }

    #[test]
    fn test_g2p_underscore_compound_matches_current_misaki_joining() {
        let g2p = G2P::new();
    ...
    #[test]
    fn test_oov_fallback_uses_initialized_espeak() {
        let g2p = G2P::with_config(G2PConfig {
    ...
    #[test]
    fn test_camel_case_matches_current_misaki_grouping() {
        let g2p = G2P::new();
        ...
        assert!(
            !result.contains(' '),
            "Unexpected camelCase space: {result}"
        );
        assert!(result.contains('s'), "Noun use should retain /s/: {result}");
