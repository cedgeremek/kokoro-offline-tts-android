...
//! processes required). OOV pronunciation uses the packaged eSpeak library.

use std::sync::OnceLock;

use fancy_regex::Regex;

use crate::stress::{punct_tag_phoneme, PUNCTS, PUNCT_TAGS};
use crate::tagger;
...
fn simple_tag(text: &str) -> &'static str {
    ...
    } else if text.strip_prefix('-').is_some_and(|digits| {
        (1..=2).contains(&digits.len()) && digits.chars().all(|c| c.is_ascii_digit())
    }) {
        // en_core_web_sm tags short signed integers as punctuation. Misaki
        // consequently emits an empty phone string for this exact shape.
        "."
...
fn split_punct(word: &str, whitespace: &str) -> Vec<MToken> {
    ...
    // spaCy separates digit-hyphen-digit telephone-like forms before Misaki's
    // number resolver (for example `555-0123` -> `555`, `-`, `0123`).
    if let Some((left, right)) = word.split_once('-') {
        ...
        {
            let mut first = MToken::new(left, "CD", "");
            first.underscore.is_head = true;
            let mut dash = MToken::new("-", "SYM", "");
            dash.underscore.is_head = true;
            let mut last = MToken::new(right, "CD", whitespace);
            last.underscore.is_head = true;
            return vec![first, dash, last];
        }
    }

    // spaCy keeps dotted initialisms and common honorific abbreviations as a
    // single token; Misaki has matching lexicon/dotted-abbreviation handling.
    let trimmed = word.trim_matches('.');
    let dotted_initialism = trimmed.contains('.')
        && word.replace('.', "").chars().all(|c| c.is_alphabetic())
        && word.split('.').map(str::len).max().unwrap_or(0) < 3;
    let honorific = word.ends_with('.')
        && matches!(
            trimmed.to_ascii_lowercase().as_str(),
            "mr" | "mrs" | "ms" | "dr" | "prof" | "sr" | "jr" | "st"
        );
    if dotted_initialism || honorific {
        let mut tok = MToken::new(word, "DEFAULT", whitespace);
        tok.underscore.is_head = true;
        return vec![tok];
    }

    // Split internal quote/dash boundaries that spaCy emits as standalone
    // punctuation tokens (for example `words”—and`).
    if word
        .chars()
        .any(|c| matches!(c, '“' | '”' | '"' | '—' | '–'))
    {
        ...
        for (index, ch) in chars.iter().copied().enumerate() {
            if matches!(ch, '“' | '”' | '"' | '—' | '–') {
                if !segment.is_empty() {
                    let completed = std::mem::take(&mut segment);
                    result.extend(split_punct(&completed, ""));
                }
                ...
                let mut token = MToken::new(text, tag, "");
                token.underscore.is_head = true;
                result.push(token);
            } else {
                segment.push(ch);
            }
        ...
        if !segment.is_empty() {
            result.extend(split_punct(&segment, whitespace));
...
pub fn tokenize(text: &str) -> Vec<MToken> {
    ...
    // The compact perceptron sometimes treats sentence-initial capital "A"
    // as a proper noun. spaCy tags it as a determiner in article position.
    for i in 0..tokens.len() {
        let starts_clause = i == 0
            || tokens[i - 1]
                .text
                .chars()
                .any(|c| matches!(c, '.' | '!' | '?' | ';' | ':'));
        let followed_by_word = tokens
            .get(i + 1)
            .is_some_and(|next| next.text.chars().any(|c| c.is_alphabetic()));
        if tokens[i].text == "A" && starts_clause && followed_by_word {
            tokens[i].tag = "DT".to_string();
        }
    }

    // Contextual corrections for two common constructions where the compact
    // tagger differs from spaCy's en_core_web_sm tags used by Misaki.
    for i in 0..tokens.len() {
        if tokens[i].text.eq_ignore_ascii_case("by")
            && ((i + 2 < tokens.len()
                && tokens[i + 1].text.eq_ignore_ascii_case("and")
                && tokens[i + 2].text.eq_ignore_ascii_case("by"))
                || (i >= 2
                    && tokens[i - 1].text.eq_ignore_ascii_case("and")
                    && tokens[i - 2].text.eq_ignore_ascii_case("by")))
        {
            tokens[i].tag = "RB".to_string();
        }
        if i >= 2
            && tokens[i].text.eq_ignore_ascii_case(&tokens[i - 1].text)
            && matches!(
                tokens[i - 2].text.to_ascii_lowercase().as_str(),
                "the" | "a" | "an"
            )
        {
            tokens[i - 1].tag = "NN".to_string();
            tokens[i].tag = "VB".to_string();
        }

        // spaCy's small English model consistently treats lowerCamelCase and
        // mixed acronym/camel identifiers as singular nouns. This distinction
        // is audible for heterographs such as noun `use` (/s/) versus verb
        // `use` (/z/).
        if tokens[i]
            .text
            .chars()
            .zip(tokens[i].text.chars().skip(1))
...
fn append_retokenized(output: &mut Vec<TokenOrGroup>, mut token: MToken) {
    if token.underscore.alias.is_some() || token.phonemes.is_some() {
        output.push(TokenOrGroup::Single(token));
    } else if let Some(TokenOrGroup::Group(group)) = output.last_mut() {
        if group
            .last()
            .is_some_and(|previous| previous.whitespace.is_empty())
        {
            token.underscore.is_head = false;
            group.push(token);
        } else if token.whitespace.is_empty() {
            output.push(TokenOrGroup::Group(vec![token]));
        } else {
            output.push(TokenOrGroup::Single(token));
        }
    } else if token.whitespace.is_empty() {
...
pub fn retokenize(tokens: Vec<MToken>) -> Vec<TokenOrGroup> {
    ...
    for tok in tokens {
        ...
        if subtokens.len() <= 1 {
                ...
                && !tok.text.chars().all(|c| c.is_alphabetic())
            {
                ...
                append_retokenized(&mut output, tok);
            }

            continue;
        }

        // Multiple subtokens: mirror Misaki's incremental grouping. Tokens
        // with aliases or pre-resolved phonemes remain standalone boundaries.
        for (i, sub_text) in subtokens.iter().enumerate() {
            ...
            if i > 0
                ...
                && subtokens[i + 1]
                    ...
                    .is_some_and(|c| c.is_alphabetic())
            {
                sub_tok.underscore.alias = Some("to".to_string());
            }
            ...
            append_retokenized(&mut output, sub_tok);
        }
    ...
    output
        .into_iter()
        .map(|item| match item {
            TokenOrGroup::Group(mut group) if group.len() == 1 => {
                TokenOrGroup::Single(group.remove(0))
            }
            item => item,
        })
        .collect()
...
mod tests {
    ...
    fn subtokenize_acronym_prefix_before_word() {
        assert_eq!(subtokenize("HTMLElement"), vec!["HTMLElement"]);
        assert_eq!(subtokenize("URLSearchParams"), vec!["URLSearch", "Params"]);
    }
