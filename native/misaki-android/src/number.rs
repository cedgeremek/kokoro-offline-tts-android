...
pub fn int_to_year(n: i64) -> String {
    // num2words' year mode treats three-digit values as a leading digit plus
    // a two-digit remainder (`123` -> `one twenty-three`). This is observable
    // in Misaki for grouped forms such as the `0123` half of a phone number.
    if (100..=999).contains(&n) {
        let leading = n / 100;
        let remainder = n % 100;
        return if remainder == 0 {
            format!("{} hundred", int_to_words(leading))
        } else if remainder < 10 {
            format!("{} oh {}", int_to_words(leading), int_to_words(remainder))
        } else {
            format!("{} {}", int_to_words(leading), int_to_words(remainder))
        };
    }
...
mod tests {
    ...
    #[test]
    fn test_year_small() {
        assert_eq!(int_to_year(999), "nine ninety nine");
    }
}
